...
export const GET: APIRoute = async () => {
  ...
  if (API_KEY) {
    try {
      ...
      console.warn('External API fetch failed, serving CORS-friendly high-res release asset feed.');
    }
  }

  // Reliable, high-speed gaming imagery with zero hotlink blocking
  const reliableReleases: GameRelease[] = [
    {
      id: 201,
  ...
  return new Response(JSON.stringify(reliableReleases), {
    status: 200,
    headers: { 'Content-Type': 'application/json' },
  });
};